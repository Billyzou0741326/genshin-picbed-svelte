...
<section class="p-4 lg:p-8">
    ...
    <div class="flex flex-row gap-2 my-4">
        <div class="w-56">
            <label class="flex flex-row items-center justify-center px-4 py-2 bg-white rounded-md shadow-md cursor-pointer text-blue-500 hover:bg-blue-500 hover:text-white">
                <div class="mr-2">
        ...
        {#if predictResult && predictResult.length > 0}
            <button class="uppercase text-sm px-4 py-2 rounded-lg outline-none focus:outline-none transition-all bg-gray-200 hover:bg-gray-400" on:click="{clearResult}">
                Clear
            </button>
        {/if}
    </div>
    ...
    {#if files && files?.length > 0}
        <div class="flex flex-col gap-2 md:w-96 px-10 py-6 mb-12 bg-white dark:bg-gray-800 rounded-lg shadow-md divide-y">
            {#each files as file, i}
                <div class="flex flex-row w-full items-center gap-2 pt-2">
                    <div class="w-4 text-center">{i+1}</div>
                    <div class="w-12 h-12"><img class="w-12 h-12 object-cover" src="{URL.createObjectURL(file)}" /></div>
                    <div class="w-32 md:w-72 overflow-hidden">{file.name}</div>
                </div>
            {/each}
            <div class="flex flex-row place-items-end gap-2 px-2 pt-4">
                <span class="flex-1"></span>
                <button class="flex-0 uppercase text-sm px-4 py-2 rounded-lg outline-none focus:outline-none transition-all bg-gray-100 hover:bg-gray-300" on:click="{clearInput}">Cancel</button>
                <button class="flex-0 uppercase text-sm px-4 py-2 rounded-lg outline-none focus:outline-none transition-all bg-blue-300 hover:bg-blue-500" on:click="{onUpload}">Upload</button>
            </div>
        </div>
    {/if}
    ...
    {:else if predictResult && predictResult.length > 0}
        <div class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-2 items-center">
            ...
            <div class="w-full h-full rounded-2xl shadow-md">
